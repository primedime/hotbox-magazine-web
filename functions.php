 ...
 */
function hotbox_magazine_setup() {

	/*
	 * Make theme available for translation.
	 * Translations can be filed at WordPress.org. See: https://translate.wordpress.org/projects/wp-themes/HotBox Magazine
	 * If you're building a theme based on Ignition, and you downloaded this from github, use a find and replace
	 * to change 'hotbox-magazine' to the name of your theme in all the template files.
	 */
	load_theme_textdomain( 'hotbox-magazine' );
    ...
    */
	...
	register_nav_menus( array(
		'top' => __( 'Top Menu', 'hotbox-magazine' ),
	) );
...
add_action( 'after_setup_theme', 'hotbox_magazine_setup' );
...
if ( ! function_exists( 'ign_widgets_init' ) ) {
	function ign_widgets_init() {
		register_sidebar( array(
			'name'          => __( 'Sidebar', 'hotbox-magazine' ),
			'id'            => 'sidebar-1',
			'description'   => __( 'Add widgets here to appear in your sidebar.', 'hotbox-magazine' ),
...
//footer widgets and sections. up to 4
		register_sidebar( array(
			'name'          => esc_html__( 'Footer', 'hotbox-magazine' ),
			'id'            => 'sidebar-2',
		...
		register_sidebar( array(
			'name'          => esc_html__( 'Footer 2', 'hotbox-magazine' ),
			'id'            => 'sidebar-3',
			'description'   => esc_html__( 'Add footer widgets here.', 'pwm' ),
			'before_widget' => '<section id="%1$s" class="widget %2$s">',
			'after_widget'  => '</section>',
		...
		register_sidebar( array(
			'name'          => esc_html__( 'Footer 3', 'hotbox-magazine' ),
			'id'            => 'sidebar-4',
			'description'   => esc_html__( 'Add footer widgets here.', 'pwm' ),
			'before_widget' => '<section id="%1$s" class="widget %2$s">',
			'after_widget'  => '</section>',
		...
		register_sidebar( array(
			'name'          => esc_html__( 'Footer 4', 'hotbox-magazine' ),
			'id'            => 'sidebar-5',
			'description'   => esc_html__( 'Add footer widgets here.', 'pwm' ),
			'before_widget' => '<section id="%1$s" class="widget %2$s">',
			'after_widget'  => '</section>',
 ...
 */
function hotbox_magazine_scripts() {
	// Add google fonts
	wp_enqueue_style( 'hotbox-magazine-fonts', ign_google_fonts_url(), array(), null );

	// Theme stylesheet. Will get this stylesheet or a child themes stylesheet.
	wp_enqueue_style( 'hotbox-magazine-style', get_stylesheet_uri(), '', '1.0' );

	//Sass compiles styles. Will get child's theme version if found instead. Child theme should import with sass.
	wp_enqueue_style( 'hotbox-magazine-sass-styles', get_theme_file_uri( '/main.css' ), '', '1.0' );
	...
	wp_enqueue_script( 'hotbox-magazine-custom-js', get_template_directory_uri() . '/assets/js/custom.js', array( 'jquery' ),
		'1.0', true );

	//AJAX ready for .custom.js files
	wp_localize_script( 'hotbox-magazine-custom-js', 'frontEndAjax', array(
		'ajaxurl' => admin_url( 'admin-ajax.php' ),
		'nonce'   => wp_create_nonce( 'ajax_nonce' ),
		'url' => home_url(),
	) );

	wp_localize_script( 'hotbox-magazine-custom-js', 'screenReaderText', array(
		'quote'    => ign_get_svg( array( 'icon' => 'quote-right' ) ),
		'expand'   => __( 'Expand child menu', 'hotbox-magazine' ),
		'collapse' => __( 'Collapse child menu', 'hotbox-magazine' )
	) );

	//Icons: add icons for use in custom js here
	wp_localize_script( 'hotbox-magazine-custom-js', 'icons', array(
...
add_action( 'wp_enqueue_scripts', 'hotbox_magazine_scripts' );
...
--------------------------------------------------------------*/
/**
 * HotBox Magazine abilities and extra filter and functions
